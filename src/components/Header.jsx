 ...
 * - Shows a cart icon with item count from CartContext.
...
const Header = ({ onSearchClick, showSearch }) => {
    const ctx = useCart();                     // could be null briefly
    const cartItems = ctx?.cartItems ?? [];    // fallback to []
    ...
    return (
        <nav className="navbar navbar-expand-lg navbar-light bg-light px-4 border-bottom border-1 border-secondary" style={{ overflow: 'visible', minHeight: '48px' }}>
            <Link className="navbar-brand d-flex align-items-center" to="/" style={{ height: '48px', overflow: 'visible', paddingTop: 0, paddingBottom: 0 }}>
                <img src="/logo.png" alt="OVRWRK Logo" height="128" style={{ marginTop: '-40px', marginBottom: '-40px', marginRight: '0.65em', display: 'block' }} />
            </Link>
            <div className="collapse navbar-collapse">
                ...
                <button className="btn btn-link p-0 me-3" style={{ fontSize: '1.6rem', color: '#222' }} aria-label={showSearch ? 'Close search' : 'Open search'} onClick={onSearchClick}>
